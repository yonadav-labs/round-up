import source.lib.shopify.resources
...
class Metafields(object):
    ...
    def metafields(self, _options=None, **kwargs):
        if _options is None:
            _options = kwargs
        return source.lib.shopify.resources.Metafield.find(resource=self.__class__.plural, resource_id=self.id, **_options)
...
class Events(object):
    ...
    def events(self):
        return source.lib.shopify.resources.Event.find(resource=self.__class__.plural, resource_id=self.id)
