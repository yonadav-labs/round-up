...
class Session(object):
    ...
    def temp(cls, domain, token):
        ...
        original_site = ShopifyResource.get_site()
        original_token = ShopifyResource.get_headers().get('X-Shopify-Access-Token')
        original_session = Session(original_site, original_token)

        session = Session(domain, token)
        ShopifyResource.activate_session(session)
        yield
        ShopifyResource.activate_session(original_session)
