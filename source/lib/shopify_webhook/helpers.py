...
def get_data_hash(domain, data):
    """
    Calculate the a hash of the body and domain name to get uniqueness for deferred task queues.
    """
    try:
        kwargs_values = [
            value for key, value in OrderedDict(
                sorted(data.items())
            ).iteritems() if not key.startswith('_')
        ]

        name_from_args = '-'.join(map(unicode, tuple(kwargs_values)))

        return str(domain) + '-' + hashlib.sha1(json.dumps(name_from_args, sort_keys=True)).hexdigest()
    except Exception as e:
        logging.critical('Exception in get_data_hash: result of data:' + str(data) + str(e.message))
        raise e
