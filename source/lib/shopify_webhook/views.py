...
from source.lib.shopify_webhook.helpers import get_signal_name_for_topic, get_data_hash
from . import signals
...
class WebhookView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Create a unique ID for the received webhook request.
        dispatch_uid = get_data_hash(domain = request.webhook_domain, data=request.webhook_data)

        try:
            signals.webhook_received.send_robust(self, domain = request.webhook_domain, topic = request.webhook_topic, data = request.webhook_data, uid = dispatch_uid)
            getattr(signals, signal_name).send_robust(self, domain = request.webhook_domain, topic = request.webhook_topic, data = request.webhook_data, uid = dispatch_uid)
